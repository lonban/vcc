...
;(function($){
    $.fn.extend({
        /*tab菜单和选项 （vcc_tab="true"父div），（vcc_tab_top="1"头部选择按扭），（vcc_tab_content="1"对应的内容区）*/
        vcc_tab:function(data)
        {
            data = $.extend({
                'event': 'click',
                'type': 1
            ...
            var TypeObj = {
                ...
                {
                    /*通过eq，订单页用*/
                    var vcc_top = obj.find('[vcc_tab_top]');
                    var vcc_content = obj.find('[vcc_tab_content]');
                    vcc_content.each(function (i) {
                        if (i != 0) {
                            $(this).hide();
                        }
                    });
                    vcc_top.each(function (i) {
                        if (data.event == 'hover') {
                            $(this).hover(function () {
                                $(this).addClass('active').siblings().removeClass('active');
                                vcc_content.eq(i).show().siblings().hide();
                            });
                        } else {
                            $(this).click(function () {
                                $(this).addClass('active').siblings().removeClass('active');
                ...
                '2':function(obj)
                {
                    ...
                    obj.find('[vcc_tab_top]').each(function () {
                        if (data.event == 'hover') {
                            $(this).hover(function () {
                                $(this).addClass('active');
                                $(this).next().show();
                                $(this).parent().siblings().find('[vcc_tab_top]').removeClass('active');
                                $(this).parent().siblings().find('[vcc_tab_content]').hide();
                            });
                        } else {
                            $(this).click(function () {
                                $(this).addClass('active');
        ...
        /*图片盒子，每一个需要图片盒子,控制图片高等于宽的,都得用一个<div class="imgDIV">包起来。调用$('.imgDIV').vcc_imgdiv();*/
        vcc_imgDiv:function(data)
        {
            data = $.extend({
                'obj':'img',
            },Default_,data,attr_json_($(this)));
        ...
        /*在父元素上加上一个class或属性用于捕捉，需要控制宽度的子元素加上vcc_divided_z="true"。调用$('.a').vcc_divided(可以用json形式传css样式进来{"border":"solid 1px #eee"});*/
        vcc_divided:function(data)
        {
            data?data:0;
            $(this).each(function(){
                data = $.extend({
                ...
                var WidthObj = {
                    'A' : function(obj)
                    {
                        var z_obj = obj.find('[vcc_divided_z]');z_obj.css(data);
                    ...
                    'vC':function(obj)
                    {
                        ...
                        obj.find('[vcc_divided_z]').css(css_json);
                    }
                };
        ...
        /*$('#left_menu').vcc_position({"top":'#top_menu',"bottom":'.footer',"type":1}); top距离顶部的距离，bottom距离底部的距离，可以是固定的数字也可以是id选择元素也就是该固定元素上面或下面的元素,type是类型1为最终定位2为实时相对定位*/
        vcc_position:function(data)
        {
            data?data:0;
            $(this).each(function(){
                data = $.extend({
        ...
        vcc_load:function(data)
        {
            data = $.extend({},Default_,data,attr_json_($(this)));
            $(this).each(function(){
                if($(this).attr('vcc_true')!=data.vcc_true2_){
                    var link = $(this).attr('vcc_load');
                    if(link){
                        $(this).load(link);
                    }
                    $(this).attr('vcc_true',data.vcc_true_);
        ...
        vcc_overflow_scroll:function(data)
        {
            data = $.extend({},Default_,data,attr_json_($(this)));
            var Vc = {
                'scroll':function(obj)
...
$(function(){
    ...
    if(node_overflow_scroll.length>0){
        node_overflow_scroll.vcc_overflow_scroll({'vcc_true_':'0'});
    }

    var node_button = $('.button');
    if(node_button.length>0){
        node_button.vcc_button({'vcc_true_':'0'});
    }
