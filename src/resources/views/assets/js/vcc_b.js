...
;(function($){
    $.fn.extend({
        /*
        （alert_class为弹出层的class属性，在父元素或传参形式配置【在多个奇葩弹出层并出现干扰时才须要配置】），（配置两个子元素分别为vcc_run触发元素，vcc_come触发后需要显示的元素）（quit为取消弹框的元素）
        checked里放需要先触发的input的父元素多个以逗号隔开，checkedZ为checked的子元素多个选择时能起作用,event_在没有触发checked里面的元素时删除的事件$('.vcc_alert').vcc_alert({'checked':'.vcc_alert_checked1,.vcc_alert_checked2','event_':'data-request'});特殊情况说明
        */
        vcc_alert:function(data)
        {
            data = $.extend({
            ...
            $(this).each(function(){
                if($(this).attr('vcc_true')!=data.vcc_true2_) {
                    var obj = $(this);
                    var run = $(this).find('[vcc_run]');/*触发（点击后）*/
                    var come = $(this).find('[vcc_come]');/*响应（遮罩层）*/
                    ...
                    $(this).attr('vcc_true',data.vcc_true_);
                }
            });
            return $(this);
        },
        /*点击弹出框*/
        vcc_alert_click:function(data)
        {
            ...
            $(this).each(function(){
                if($(this).attr('vcc_true') != data.vcc_true2_) {
                    var obj = $(this);
                    /*它们也可以共同放在一个元素上面*/
                    var content = obj.find('[vcc_alert_content]');/*弹出内容*/
                    var alert_click = obj.find('[vcc_alert_click]');/*点击*/

                    if(alert_click.length>0){
                        alert_click.click(function(){
                            /*点击后弹出或隐藏content*/
                            if($(this).attr('vcc_alert_click') == 'show'){
        ...
        vcc_alert_open:function(data)
        {
            ...
            $(this).each(function(){
                if($(this).attr('vcc_true') != data.vcc_true2_) {
                    ...
                    /*它们也可以共同放在一个元素上面*/
                    var content = obj.find('[vcc_alert_content]');/*弹出内容*/
                    var click_show = obj.find('[vcc_alert_click="show"]');/*点击*/
                    var click_hide = obj.find('[vcc_alert_click="hide"]');/*点击*/
        ...
        /*购物车数量的统计与提交*/
        /*div[vcc_cart_update="{{ cart.id }}"]>button[vcc_update="minus"]+input[name="pn[{{ cart.id }}]"]+button[vcc_update="plus"]*/
        vcc_2_cartUpdate:function (data)
        {
            data = $.extend({
                'is_ajax':'yes'
            },Default_,data,attr_json_($(this)));
            ...
            function ajax_(obj)
            {
                if(data.is_ajax == 'yes'){
                    ...
                    $.post(url,send_data,function(){
                        $('#vcc_order_total').load(location.href+" #vcc_order_total");
                    });
                }
            }
            $(this).each(function () {
                if ($(this).attr('vcc_true')!=data.vcc_true2_) {
                    var obj = $(this);
                    var obj_c = obj.find('[vcc_update]');
                    var input = obj.find('input');
                    obj_c.on('click',function(){
                        var val = parseInt(input.val());
                        var type = $(this).attr('vcc_update');
                        if(type == 'plus'){
                            input.val(val+1);
                        }else if(type == 'minus'){
                            if(input.val()>1){
        ...
        /*评星，评论五角星*/
        /*span[vcc_score="rating"]>a[class="iconfont"]{&#xe707;}*/
        vcc_2_score: function (data) {
            data = $.extend({},Default_,data,attr_json_($(this)));
            $(this).each(function () {
                if ($(this).attr('vcc_true') != data.vcc_true2_) {
                    var obj = $(this);
                    var input = $('[name='+obj.attr('vcc_score')+']');
        ...
        /*验证最后一位不通过自动删除最后一位,一般用于电话vcc_tell="tel"或邮编vcc_tell="zip"*/
        vcc_form_auto:function(data)
        {
            data = $.extend({
                're':/[^0-9]+/g
            },Default_,data,attr_json_($(this)));
        ...
        /*验证长度，必须有vcc_tell属性，在有vcc_length="6,16"最少长度,最大长度，只有一位如vcc_length="16"则只验证最大长度*/
        vcc_form_length:function(data)
        {
            data = $.extend({
                /*'length':{"min":4,"max":16},/!*电话*!/*/
                /*'length':{"min":6,"max":200},/!*邮箱*!/*/
                'length':{"min":4,"max":200},/*综合默认长度限制，vcc_length="这里面没有内容才会触发默认长度限制"*/
        ...
        /*验证不通过阻止提交vcc_tell="form"写在表单上或表单父元素上*/
        vcc_form_submit:function(data)
        {
            data = $.extend({},Default_,data,attr_json_($(this)));
            var form_submit = {
                ...
                'alert_add2':function(form,hint,time,click_remove)
                {
                    ...
                    {
                        if(form.attr('vcc_tell_is') == 'submit'){
                            form.removeAttr('vcc_tell_is');
                        }
            ...
            if($(this).attr('vcc_true')!=data.vcc_true2_) {
                $(this).each(function () {
                    var form = $(this);
                    if(form.attr('vcc_tell') != 'break'){
                        form.submit(function (e) {
                            var submit = Form_.Other.submit(form);
                            if(submit['vcc_tell_is'] == 'error'){
                                form_submit.alert_add(submit['hint'],3300,true);
                                return false;
                            }else if(submit['vcc_tell_is'] == 'submit'){
                                form_submit.alert_add(submit['hint'],88888,true);
                                return false;
                            }else{
                                form.attr('vcc_tell_is','submit');
                                $._data(form, 'e', null);
                            }
                        });

                        form.on('change',function(){
                            form.attr('vcc_tell_is','pass');
                        });
                    }
                });
                $(this).attr('vcc_true', data.vcc_true_);
        ...
        /*表单验证集合*/
        vcc_tell:function(data)
        {
            data = $.extend({/*'tell_type':['tel','zip','email']*/},Default_,data,attr_json_($(this)));
            /*for(var i=0;i<data.tell_type.length;i++){
                if($(this).attr('vcc_tell') == data.tell_type[i]){
                    $(this).find('input').'vcc_form_'+data.tell_type[i]();
                }
            }*/

            $(this).each(function(){
                if($(this).find('input').length > 0){
                    switch($(this).attr('vcc_tell')){
                        case 'tel':$(this).find('input').vcc_form_auto();
                            break;
                        case 'zip':$(this).find('input').vcc_form_auto();
                            break;
                        case 'email':$(this).find('input').vcc_form_email();
                            break;
                    }
                    $(this).attr('vcc_length') && $(this).find('input').vcc_form_length();
                }else{
                    switch($(this).attr('vcc_tell')){
                        case 'tel':$(this).vcc_form_auto();
                            break;
                        case 'zip':$(this).vcc_form_auto();
                            break;
                        case 'email':$(this).vcc_form_email();
                            break;
                    }
                    $(this).attr('vcc_length') && $(this).vcc_form_length();
                }

                if($(this).attr('vcc_tell') == 'form'){
                    if($(this)[0].tagName == 'FORM'){
                        $(this).vcc_form_submit();
                    }else{
                        var form = $(this);
                        ...
                        form.find('form').vcc_form_submit();
                    }
                }
            });
            return $(this);
        ...
        vc2_autoLogin:function(data){
            ...
            var Form = {
                'form':form,
                'hint':form.find('[vcc_auto-user="hint"]'),
                'tel':form.find('[vcc_auto-user=tel]'),/*self:<vcc_auto-user="hint">*/
                'email':form.find('[vcc_auto-user=email]'),/*self:<vcc_auto-user="hint">*/
                'address':form.find('[vcc_auto-user="address"]'),/*children:<select>*/
                'button':form.find('[vcc_auto-user=button]'),/*children:<vcc_tell="children_submit">*/
                'password':form.find('[vcc_auto-user=password]'),
            };
            var Form_c = {
                ...
                'button':Form.button.find('[vcc_tell]'),
                    ...
                    $.ajax({
                        ...
                        success : function(data){
                            ...
                            if (data['data'] > 0) {
                                ...
                                Form.button.show();
                                Form.password.show();
                                Form_c.button.attr('vcc_tell_is','error');
                                ...
                                Form.button.hide();
                                Form.password.hide();
                                Form_c.button.attr('vcc_tell_is','pass');
                                Form_c.password.val(Form_c.tel.val()).attr('name','password');
                                ...
                                if(address(Form_c.address.val()) == 'error'){
                                    ...
                                    /*隐藏必填图标并自动填充默认邮箱*/
                                    Form_c.email_i.hide();
                                    Form_c.email.removeAttr('required').attr('vcc_tell_is','pass');
                                    if(type == 'tel'){
            ...
            if($(this).attr('vcc_true')!=data.vcc_true2_) {
                ...
                Form.form.find('input').not(Form_c.password).not(Form_c.email).not(Form_c.tel).on('focus',function(){
                    if(Form_c.button.attr('vcc_tell_is') == 'error'){
                        Fill.ajax_signIn(Form_c);
                    }
                });
                $(this).attr('vcc_true',data.vcc_true_);
    ...
    var Form_ = {
        ...
        'Length':function(obj)
        {
            var length = obj.attr('vcc_length');
            var min = null;
            var max = null;
            var state = null;
            if(length){
        ...
        'Other':{
            'radio':function(form)
            {
                var obj = form.find('[vcc_tell="radio"]');
                var radio_data = null;
                if(obj.length>0){
                    var name = {};
                    obj.each(function () {
                    ...
                    $.each(name,function(key){
                        radio_data[key] = {'hint':'Please check the options','vcc_tell_is':'radio'};
                        var obj_c = $('[name="'+key+'"]');
                        obj_c.eq(0).attr('vcc_tell_is','radio');
                        obj_c.each(function(){
                            ...
                            if($(this).prop('checked')){
                                radio_data[key]['vcc_tell_is'] = 'pass';
                                obj_c.eq(0).attr('vcc_tell_is','pass');
                            }
                        });
                    });
                }
            ...
            'submit':function(form)
            {
                ...
                form.find('[vcc_tell]').addBack().each(function (i) {
                    var hint = null;
                    var vcc_tell_is = $(this).attr('vcc_tell_is');
                    switch (vcc_tell_is) {
                        case 'length':
                ...
                var arr={'hint':'Radio please select','vcc_tell_is':'pass','vcc_tell_type':null};
                $.each(error_arr,function(i){
                    if(i != 'submit' && error_arr[i]){
                        arr['hint'] = error_arr[i];
                        arr['vcc_tell_is'] = 'error';
                    }
                });
                if(arr['vcc_tell_is'] == 'pass' && error_arr['submit']){
                    arr['hint'] = error_arr['submit'];
                    arr['vcc_tell_is'] = 'submit';
                }
                return arr;
            }
        }
    ...
    var Alert2_ = {
        'id':{'relative1':'vcc__alert_relative1','body1':'vcc__alert_body1'},
...
$(function(){
    /*五角星*/
    var vcc_score = $('[vcc_score]');
    if(vcc_score.length>0){
        vcc_score.vcc_2_score();
    }

    /*表单验证*/
    var vcc_tell = $('[vcc_tell]');
    if(vcc_tell.length>0){
        vcc_tell.vcc_tell();
    }
});
/*拒絕修改*/